...
export async function getMonthlyTransactionsByStatus(status) {
    const { data, error } = await supabase.from("transaction_details").select("created_at, status").eq("status", status);
