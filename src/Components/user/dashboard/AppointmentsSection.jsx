...
// Sub-component to ensure React hooks are called at the top level of a component
const AppointmentCard = ({ appointment, handleViewLetter, handlePrint, getStatusColor, getStatusIcon }) => {
  const { data: countryDetails } = useFullCountryDetails(appointment?.country_id);

  const reasons = (appointment?.appointment_reason || []).map(res => {
    return getFallbackReasonById(res)?.type || res;
  });
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          <p className="text-gray-600 text-sm mb-4">{reasons.length > 0 ? reasons.join(", ") : 'N/A'}</p>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="text-gray-600">
                ...
                {appointment?.applied_at ? new Date(appointment.applied_at).toLocaleDateString("en-GB") : 'N/A'}
              </span>
            </div>
            ...
            <div className="text-sm text-gray-600">
              ...
              <div className="mt-3 rounded-lg border border-gray-200 bg-gray-50 p-4 space-y-2">
                <h4 className="text-sm font-semibold text-gray-800">
                  {embassy.name || 'Embassy Office'}
                </h4>

                {embassy.address && (
                  <p className="text-sm text-gray-600">
                    <MapPinned className='inline h-4 mr-1' /> {embassy.address}
                  </p>
                )}

                {embassy.contact_no && (
                  <p className="text-sm text-gray-600">
                    <Phone className='inline h-4 mr-1' /> {embassy.contact_no}
                  </p>
                )}

                {embassy.website_url && (
                  <a
                    href={embassy.website_url.startsWith('http') ? embassy.website_url : `https://${embassy.website_url}`}
                    target="_blank"
        ...
        <div className="flex items-center gap-2 ml-4">
          <button
            onClick={() => handleViewLetter(appointment, countryDetails, reasons)}
          ...
          <button
            onClick={() => handlePrint(appointment, countryDetails, reasons)}
            className="p-2 hover:bg-gray-100 rounded-lg transition-colors cursor-pointer"
